...
2. 点面板「复制会话」→ JSESSIONID 进剪贴板（个别 Surge 版本无剪贴板 API 时，
   通知改为显示完整会话，长按通知「拷贝」即可）→ 打开 QLIT「出行登记」粘贴导入；
3. 在 QLIT 里完成表单提交；或直接点面板「出行登记」，三五秒后收到结果通知；
4. 会话失效时的提示是「会话过期，请去微信刷新登录态」，重新登录一次即可闭环。
