/**
 * QLIT 出行登记 · 提交面板脚本（generic / 磁贴）
 *
 * 流程：读取已捕获会话 → SSO 换 JWT → 读配置与已有登记
 *      → 用下方 FORM 组装提交体 → POST 提交 → 通知 + 面板回显结果。
 *
 * 点击面板即视为最终确认（对应 Android 版的确认弹窗），脚本不会自动定时提交。
 */
...
/** 对齐 Mac 版 parse_message：message 可能是对象或 JSON 字符串 */
...
/**
 * 由服务端配置 + 已有记录组装提交体；数据不足/缺字段时直接抛错（消息给用户看）。
 */
function buildBody(configMsg, records, form, todayIso) {
  ...
  if (missing.length) throw new Error('请在 qlit-submit.js 顶部补全默认表单值：\n' + missing.join('、'));
  ...
  if (sameDay.length && !form.ALLOW_DUPLICATE) {
    throw new Error('今天(' + todayIso + ')已有 ' + sameDay.length + ' 条登记。' +
      '如确需重复提交，把 qlit-submit.js 里 ALLOW_DUPLICATE 改为 true');
  }
...
if (typeof $httpClient !== 'undefined' && typeof $persistentStore !== 'undefined') {
  ...
  function httpGet(url, headers, cb) {
    $httpClient.get({ url: url, headers: headers }, function (status, resp, data) {
      cb(String(status), resp, data);
    });
  }
  function httpPost(url, obj, headers, cb) {
    var h = headers;
    h['Content-Type'] = 'application/json';
    $httpClient.post({ url: url, headers: h, body: JSON.stringify(obj) }, function (status, resp, data) {
      cb(String(status), resp, data);
    });
  }
  ...
  function runSubmit() {
    ...
    if (!jsid) {
      ...
      finish({ title: '出行登记', content: '未捕获会话，请先在微信登录平台' });
      return;
    }

    // 并发锁：面板可能被连点或与其他触发方式叠加
    var now = Date.now();
    var lockAt = Number(store.read(KEY_LOCK) || 0);
    if (now - lockAt < LOCK_MS) {
      finish({ title: '出行登记', content: '上一轮仍在进行中…' });
      return;
    }
    ...
    httpGet(SSO_URL, headersFor(jsid), function (st1, _r1, html) {
      jwt = jwtOf(html);
      if (!jwt) {
        store.write('', KEY_LOCK);
        notify('✗ 会话已过期', 'SSO 未返回授权令牌。\n请在微信里重新打开一次在校生服务平台，再回来提交。');
        finish({ title: '出行登记', content: '会话过期，请去微信刷新登录态' });
        return;
      }

      httpGet(CONFIG_URL, headersFor(jsid, jwt), function (_st2, _r2, cfgText) {
        var cfgMsg;
        try { cfgMsg = unwrapMessage(JSON.parse(cfgText)); } catch (e) {
          store.write('', KEY_LOCK);
          notify('✗ 配置解析失败', String(e).slice(0, 200));
          finish({ title: '出行登记', content: '读取出行配置失败' });
          return;
        }

        httpGet(RECORDS_URL, headersFor(jsid, jwt), function (_st3, _r3, recText) {
          var records = [];
          try {
          ...
          try { built = buildBody(cfgMsg, records, FORM, todayIso()); } catch (e) {
            ...
            finish({ title: '出行登记', content: String(e).split('\n')[0] });
            return;
          }

          httpPost(SUBMIT_URL, built.body, headersFor(jsid, jwt), function (_st4, _r4, subText) {
            store.write('', KEY_LOCK);
            var ok = false, detail = '';
            try {
            ...
            if (ok) {
              ...
              finish({ title: '出行登记 ✓', content: built.body.CXRQ + ' 提交成功' });
            } else {
              notify('✗ 提交失败', detail || '未知原因');
              finish({ title: '出行登记 ✗', content: detail || '提交失败' });
            }
          });
        });
      });
